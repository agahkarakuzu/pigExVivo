...
%gval = 2.3;
% gval = 1.5; % Suspected outliers removal 
%% compute
for column = 1:p
    if p>1
...
if fig_flag ~= 0
    ...
    if p > 1
        ...
        if nargout == 4
            ...
            M = sprintf('Skipped correlation \n Pearson r=%g CI=[%g %g] pval=%g \n Spearman r=%g CI=[%g %g]',r.Pearson,CI.Pearson(1),CI.Pearson(2),pval,r.Spearman,CI.Spearman(1),CI.Spearman(2));
        ciOut = [CI.Pearson(1) CI.Pearson(2)];
        assignin('base','ciOut',ciOut);
        else
        ...
        try
            ...
            ellipseX = real(XEmin); ellipseY = real(YEmin); 
            assignin('base','ellipseX',ellipseX);
            assignin('base','ellipseY',ellipseY);
